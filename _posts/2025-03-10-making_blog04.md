...
```markdown
![Desktop View](/assets/img/sample.png){: width="350" class="normal"}
![Desktop View](/assets/img/sample.png){: width="350" class="left"}
![Desktop View](/assets/img/sample.png){: width="350" class="right"}
```
...
liquid code를 추가하고 싶으면 `{_% raw %}` 와 `{_% endraw %}` 에서 언더바를 제거하고 감싸주면 된다.
...
⇒ 페이지에 적용되려면 시간이 걸리는 듯 하다. 5~10분 걸린다고 알려짐