---
title: "[깃허브 블로그-07] 이미지 이름 일괄 수정 업그레이드"
date: 2025-11-02 14:37:16 +0900 # 대한민국 기준
categories: [Blog]
...
1. `/_posts/` 폴더 아래에 포스팅할 글의 내용을 작성한다.(Only text)
2. 글에 들어갈  Image들은 글의 제목과 동일한 폴더에 들어간다.
`/assets/img/posts/(글 제목)`
3. Notion의 텍스트를 그대로 복사하면 Image 경로가 다르기 때문에 이를 수정해야 한다.

## 자동화 스크립트 작성

포스팅할 글 파일을 새롭게 생성하면 자동으로 동일한 이름의 폴더를 이미지 경로에 만든다.

그리고 글을 저장할 때마다 이미지 경로를 자동으로 수정해준다. 이때 수정하는 방식은 이미지가 나온 순서대로 넘버링을 해주는 방식이다.

파일을 실행하면 실시간으로 파일 생성 및 수정을 감시한다.

![image.png](../assets/img/posts/2025-11-02-blog/image.png)

`/_posts/` 경로에 새로운 파일을 추가하면 이를 감지하고 `/assets/img/posts/(글 제목)/`  폴더를 생성한다.

![image.png](../assets/img/posts/2025-11-02-blog/image 1.png)
...
def process_new_post(post_filepath):
    ...
    try:
        ...
        # 정규식 패턴: ![모든문자](../assets/img/posts/2025-11-02-blog/image 2.png)
